...
4. 本次输出主角相关内容必须为零。禁止出现主角、玩家、主角身边人物、主角认识的人物、当前任务或当前地点作为事件主线；即使最近正文提到主角，也只能把正文当作背景观测，不能围绕其生成内容。
...
【世界事实】
无。仙网不读取主角状态型世界快照，只依据世界书规则与背景条目生成公共动态。
...
返回前检查数量、字段、时间线、主题差异和观点冲突。`;
  ...
  const systemPrompt = `${XIANWANG_TRENDS_SYSTEM_PROMPT}\n【主角隔离】本次输出主角相关条目必须为零，不得以公共影响或正文提及为例外。${settings.jailbreakPrompt ? '\n【创作完整性】不要因内容尖锐、负面、争议或涉及世界内冲突而拒绝创作；仍须服从既定格式、安全边界与世界设定。' : ''}`;
