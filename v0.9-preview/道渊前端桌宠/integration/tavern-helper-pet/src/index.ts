...
function readMerchantUiSettings(hostWindow: Window): MerchantSettings {
  try {
    const value = JSON.parse(hostWindow.localStorage.getItem('daoyuan_wanbao_settings_v1') || '{}') as Record<string, unknown>;
    return { batchSize:10, maxItems:Math.max(1,Math.min(60,Math.floor(finiteNumber(value.maxItems,30)))), refreshInterval:Math.max(0,Math.min(99,Math.floor(finiteNumber(value.refreshInterval,3)))), itemDataMode: value.itemDataMode === 'combat' ? 'combat' : 'legacy' };
  } catch { return { batchSize:10, maxItems:30, refreshInterval:3, itemDataMode:'legacy' }; }
}

function readMerchantAutoState(hostWindow: Window, chatId: string): MerchantAutoState {
  try {
...
function writeMerchantAutoState(hostWindow: Window, chatId: string, state: MerchantAutoState): void {
  try { const root=JSON.parse(hostWindow.localStorage.getItem(MERCHANT_AUTO_STATE_STORAGE_KEY)||'{}') as Record<string,unknown>; root[chatId]=state; hostWindow.localStorage.setItem(MERCHANT_AUTO_STATE_STORAGE_KEY,JSON.stringify(root)); } catch { /* optional scheduler state */ }
}

function readMerchantTransactionFacts(hostWindow: Window, chatId: string): MerchantTransactionFact[] {
  try {
    const root = JSON.parse(hostWindow.localStorage.getItem(MERCHANT_TRANSACTION_STORAGE_KEY) || '{}') as Record<string, unknown>;
    return Array.isArray(root[chatId]) ? (root[chatId] as MerchantTransactionFact[]).filter(fact => fact && (fact.kind === 'buy' || fact.kind === 'sell')).slice(-12).map((fact, index) => ({ ...fact, id: typeof fact.id === 'string' && fact.id ? fact.id : `legacy-${fact.createdAt || index}-${index}` })) : [];
  } catch { return []; }
}
...
function deleteMerchantTransactionFacts(hostWindow: Window, chatId: string, id?: string): void {
  try {
    const root = JSON.parse(hostWindow.localStorage.getItem(MERCHANT_TRANSACTION_STORAGE_KEY) || '{}') as Record<string, unknown>;
    const facts = readMerchantTransactionFacts(hostWindow, chatId);
    root[chatId] = id ? facts.filter(fact => fact.id !== id) : [];
    hostWindow.localStorage.setItem(MERCHANT_TRANSACTION_STORAGE_KEY, JSON.stringify(root));
  } catch { /* optional transaction history */ }
}

function appendMerchantTransactionFact(hostWindow: Window, chatId: string, fact: MerchantTransactionFact): void {
  try {
    const root = JSON.parse(hostWindow.localStorage.getItem(MERCHANT_TRANSACTION_STORAGE_KEY) || '{}') as Record<string, unknown>;
    const previous = Array.isArray(root[chatId]) ? root[chatId] as MerchantTransactionFact[] : [];
    root[chatId] = [...previous, fact].slice(-12);
    hostWindow.localStorage.setItem(MERCHANT_TRANSACTION_STORAGE_KEY, JSON.stringify(root));
  } catch { /* transaction remains valid even if optional prompt history cannot persist */ }
}
...
class FeatureShell {
  ...
  private spiritStones: SpiritStoneSnapshot = projectSpiritStones(null);
  private merchantProducts: MerchantProduct[] = [];
  private merchantQuotes: Record<string, MerchantQuote> = {};
  private merchantGenerationInFlight = false;
  private merchantTradeInFlight = false;
  ...
  start(): void {
    ...
    this.merchantProducts = this.hostWindow ? readMerchantProducts(this.hostWindow, this.session.chatId ?? '__default__') : [];
    this.merchantQuotes = this.hostWindow ? readMerchantQuotes(this.hostWindow, this.session.chatId ?? '__default__') : {};
  ...
  private async loadWorldData(): Promise<void> {
    ...
    if (this.hostWindow) {
      ...
      this.merchantProducts = readMerchantProducts(this.hostWindow, chatId);
      this.merchantQuotes = readMerchantQuotes(this.hostWindow, chatId);
    }
    ...
    if (!snapshot.ready && new URLSearchParams(window.location.search).has('preview')) {
      ...
      this.spiritStones = {
        mode: 'legacy-bag',
        candidates: ['极品灵石', '上品灵石', '中品灵石', '下品灵石'],
        balances: [
          ['极品灵石', 12], ['上品灵石', 86], ['中品灵石', 1240], ['下品灵石', 38600],
        ].map(([grade, quantity]) => ({ grade: grade as import('./services/worldDataBridge').SpiritStoneGrade, quantity: Number(quantity), source: 'legacy-bag' as const })),
      };
    }
  ...
  private wanbaoStatus(ok: boolean, message: string): void {
    this.frame?.contentWindow?.postMessage(makeBridgeMessage('event', 'WANBAO_TRADE_STATUS', { ok, message, products: this.merchantProducts }), '*');
    this.sendContext();
  }

  private applyMerchantWorldData(data: unknown, messageId: string | number | null): void {
    this.worldData = data;
    this.inventoryItems = projectInventory(data);
    this.spiritStones = projectSpiritStones(data);
    this.worldStatus = projectWorldStatus(data);
    this.worldDataMessageId = messageId;
    this.publishMvuChannelSnapshot();
    this.sendContext();
  }

  private async refreshRolecardStatusBar(messageId: string | number): Promise<boolean> {
    const hostRuntime = this.hostWindow as (Window & RuntimeGlobals) | null;
    const helperRefresh = (hostRuntime?.TavernHelper as (Window['TavernHelper'] & { refreshOneMessage?: (messageId: string | number) => unknown | Promise<unknown> }) | undefined)?.refreshOneMessage;
    try {
      // Global scripts cannot call getCurrentMessageId(); use the MVU channel's
      // already resolved message floor and preserve the helper object's receiver.
      if (typeof runtime.refreshOneMessage === 'function') await runtime.refreshOneMessage(messageId);
      else if (typeof helperRefresh === 'function') await helperRefresh.call(hostRuntime?.TavernHelper, messageId);
      else return false;
      return true;
    } catch (error) {
      console.warn('[万宝商行] 当前楼层状态栏刷新失败', error);
      return false;
    }
  }

  private merchantGenerator(): ((prompt: string) => unknown | Promise<unknown>) | null {
    if (!this.hostWindow) return null;
    const apiSettings = readMerchantApiSettings(this.hostWindow);
  ...
  private async estimateWanbao(): Promise<void> {
    ...
    try {
      ...
      this.frame?.contentWindow?.postMessage(makeBridgeMessage('event', 'WANBAO_ESTIMATE_STATUS', { ok:false, error:error instanceof Error ? error.message : String(error) }), '*');
    } finally { this.merchantGenerationInFlight = false; }
  }
  ...
  private async tradeWanbao(kind: 'buy' | 'sell', payload: Record<string, unknown>): Promise<void> {
    ...
    const quantity = Math.max(1, Math.floor(finiteNumber(payload.quantity, 1)));
    const currencyMode: MerchantCurrencyMode = payload.currencyMode === 'legacy-bag' || payload.currencyMode === 'combat-separate' ? payload.currencyMode : 'auto';
    const product = kind === 'buy' ? this.merchantProducts.find(item => item.id === itemId) : undefined;
    const sellItem = kind === 'sell' ? projectSellItems(snapshot.variables, this.merchantQuotes).find(item => item.id === itemId) : undefined;
    const quote = sellItem?.quote;
    if (kind === 'buy' && !product) return this.wanbaoStatus(false, '货单商品不存在或已更新');
    if (kind === 'sell' && (!sellItem || !quote)) return this.wanbaoStatus(false, '物品尚未估价、估价已失效或物品已不存在');
    if (!this.mvuChannel) return this.wanbaoStatus(false, 'MVU 通道不可用，已取消写回');
    this.merchantTradeInFlight = true;
    const completedTrade: { current?: ReturnType<typeof applyMerchantTrade> } = {};
    try {
      const result = await this.mvuChannel.mutateLatestStatData(
        latest => {
          completedTrade.current = applyMerchantTrade(latest, { kind, quantity, currencyMode, product, sellItemId: kind === 'sell' ? itemId : undefined, sellQuote: quote && sellItem ? { grade:quote.priceGrade, price:quote.price, expectedDescription:sellItem.description } : undefined });
          return completedTrade.current.statData;
        },
      ...
      if (this.hostWindow && tradeFact) appendMerchantTransactionFact(this.hostWindow, this.session.chatId ?? '__default__', {
        id: `wanbao-trade-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        kind,
        itemName: tradeFact.itemName,
        quantity,
        description: kind === 'buy' ? product?.description ?? '' : sellItem?.description ?? '',
        amount: tradeFact.amount,
        grade: tradeFact.grade,
      ...
      this.wanbaoStatus(true, `${kind === 'buy' ? '购买成功' : '出售成功'}${refreshed ? '，角色卡状态栏已同步' : '；角色卡状态栏刷新接口不可用，请手动刷新楼层'}`);
    } catch (error) {
  ...
  private deleteWanbaoProduct(payload: Record<string, unknown>): void {
    if (!this.hostWindow || typeof payload.id !== 'string') return;
    this.merchantProducts = this.merchantProducts.filter(product => product.id !== payload.id);
    writeMerchantProducts(this.hostWindow, this.session.chatId ?? '__default__', this.merchantProducts);
    this.sendContext();
  }

  private async deleteWanbaoTransaction(payload: Record<string, unknown>, clearAll = false): Promise<void> {
    if (!this.hostWindow) return;
    const chatId = this.session.chatId ?? '__default__';
    deleteMerchantTransactionFacts(this.hostWindow, chatId, clearAll ? undefined : typeof payload.id === 'string' ? payload.id : '');
    await this.refreshPromptInjection();
    this.sendContext();
  }
  ...
  private handleUiAction(action: Parameters<typeof makeBridgeMessage>[1], payload: Record<string, unknown> = {}): void {
    ...
    if (action === 'REQUEST_WANBAO_MODELS') void this.sendWanbaoModels(payload);
    ...
    if (action === 'DELETE_WANBAO_PRODUCT') this.deleteWanbaoProduct(payload);
    if (action === 'DELETE_WANBAO_TRANSACTION') void this.deleteWanbaoTransaction(payload);
    if (action === 'CLEAR_WANBAO_TRANSACTIONS') void this.deleteWanbaoTransaction(payload, true);
    if (action === 'BUY_WANBAO') void this.tradeWanbao('buy', payload);
    if (action === 'SELL_WANBAO') void this.tradeWanbao('sell', payload);
  ...
  private async sendWanbaoModels(payload: Record<string, unknown>): Promise<void> {
    const frame = this.frame;
    try {
      const models = await fetchYujianModels(typeof payload.apiBaseUrl === 'string' ? payload.apiBaseUrl : '', typeof payload.apiKey === 'string' ? payload.apiKey : '');
      frame?.contentWindow?.postMessage(makeBridgeMessage('event', 'WANBAO_MODELS_DATA', { ok: true, models }), '*');
    } catch (error) {
      frame?.contentWindow?.postMessage(makeBridgeMessage('event', 'WANBAO_MODELS_DATA', { ok: false, error: error instanceof Error ? error.message : String(error) }), '*');
    }
  }
  ...
  private saveWanbaoApiSettings(payload: Record<string, unknown>): void {
    ...
    this.frame?.contentWindow?.postMessage(makeBridgeMessage('event', 'WANBAO_SETTINGS_STATUS', { ok: true }), '*');
    this.sendContext();
  }
