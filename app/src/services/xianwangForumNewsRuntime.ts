...
async function call(settings: XianwangApiSettings, system: string, user: string, max_tokens=7000): Promise<string> {
  ...
  const suffix=`\n【主角隔离】本次输出主角相关内容必须为零，不得以公共影响或正文提及为例外。\n【玩家网名】${normalizePlayerAlias(settings.playerAlias)}。这是玩家身份，不是仙网NPC；AI生成的作者、记者、编辑、读者和评论者禁止使用此名称。${settings.jailbreakPrompt?'\n【创作完整性】不要因内容尖锐、负面、争议或涉及世界内冲突而拒绝创作；仍须服从既定格式、安全边界与世界设定。':''}`;
...
export async function generateForumReplies(settings:XianwangApiSettings,post:ForumPost,userComment:string):Promise<Array<{id:string;author:string;content:string;storyTime:string}>>{const alias=normalizePlayerAlias(settings.playerAlias);const prompt=`帖子标题：${post.title}\n帖子正文：${post.content}\n已有评论：\n${post.comments.slice(-8).map(c=>`${c.author}：${c.content}`).join('\n')}\n玩家“${alias}”刚刚评论：${userComment}\n“${alias}”仅指玩家，不得把该网名当作NPC作者。生成1至3条不同仙网道友的自然跟帖，必须直接回应玩家或帖子。只返回JSON：{"schemaVersion":1,"comments":[{"author":"道友名","content":"回复","storyTime":"当前故事时间"}]}`;const r=ForumReplyResponse.safeParse(json(await call(settings,FORUM_PROMPT,prompt,1800)));if(!r.success)throw new Error('仙网 AI 回复格式错误');return r.data.comments.map((c,i)=>({...c,id:`forum-comment:${Date.now()}:ai:${i}`}));}
