import { mutateWorldStatData, readWorldDataSnapshot, replaceWorldStatData, type MvuReadRuntime, type WorldDataWriteResult } from './worldDataBridge';
...
export class MvuChannelTool {
  ...
  async mutateLatestStatData(
    mutate: (latestStatData: Record<string, unknown>) => Record<string, unknown>,
    expected: Pick<MvuChannelContext, 'contextRevision' | 'messageId'> = this.snapshot,
  ): Promise<WorldDataWriteResult> {
    const current = this.getContext();
    if (expected.messageId === null || current.contextRevision !== expected.contextRevision || current.messageId !== expected.messageId) {
      return { available: false, written: false, contextRevision: current.contextRevision, messageId: current.messageId, data: null, reason: 'mvu-write-failed' };
    }
    const result = await mutateWorldStatData(this.getRuntime(), current.contextRevision, expected.messageId, mutate);
    if (result.written) await this.readLatestVariables();
    return result;
  }
}
