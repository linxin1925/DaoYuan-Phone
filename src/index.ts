...
import { getWorldDataCapability, projectInventory, projectNpcContacts, projectWorldStatus, projectYujianAffections, type InventoryItemSnapshot, type WorldStatusSnapshot, type YujianContactSnapshot } from './services/worldDataBridge';
import { MVU_CHANNEL_NAME, MvuChannelTool } from './services/mvuChannel';
import { fetchYujianModels, loadStandaloneKnownContacts, loadStandaloneYujianHistories, rememberStandaloneKnownContacts, resetYujianRuntimeContext, sendYujianMessageWithProgress } from './services/yujianRuntime';
...
import { generateForumPosts, generateNewsPapers, retainNewest } from './services/xianwangForumNewsRuntime';
...
function readYujianSettings(hostWindow: Window): Record<string, string> {
  try {
    ...
    return { customPrompt: DEFAULT_BEAUTY_RANK_PROMPT, apiBaseUrl: '', apiKey: '', apiModel: '' };
  }
}
...
function readBeautyApiSettings(hostWindow: Window): BeautyRankApiSettings {
  try {
    ...
    return { apiBaseUrl: '', apiKey: '', apiModel: '', autoInterval: 1 };
  }
}
...
function readXianwangApiSettings(hostWindow: Window): XianwangApiSettings {
  try {
    ...
    return { apiBaseUrl: '', apiKey: '', apiModel: '', autoInterval: 3, batchMin: 2, batchMax: 3, maxPosts: 30, forumAutoInterval: 3, forumBatchSize: 2, forumMaxPosts: 30, newsAutoInterval: 5, newsBatchSize: 1, newsMaxPapers: 12 };
  }
}
...
class FeatureShell {
  ...
  start(): void {
    ...
    this.orb.innerHTML = `
      <svg class="jade-slip-icon" viewBox="0 0 64 80" aria-hidden="true" focusable="false">
        <defs>
          <linearGradient id="jade-slip-fill" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stop-color="#a9d7b7" />
            <stop offset="0.46" stop-color="#6fb39a" />
            <stop offset="1" stop-color="#315e59" />
          </linearGradient>
          <linearGradient id="jade-slip-edge" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0" stop-color="#d8f1d0" />
            <stop offset="1" stop-color="#6a9f8a" />
          </linearGradient>
        </defs>
        <path class="jade-slip-shadow" d="M18 9h28c4 0 7 3 7 7v48c0 4-3 7-7 7H18c-4 0-7-3-7-7V16c0-4 3-7 7-7Z" />
        <path class="jade-slip-body" d="M19 8h26c4 0 7 3 7 7v50c0 4-3 7-7 7H19c-4 0-7-3-7-7V15c0-4 3-7 7-7Z" />
        <path class="jade-slip-edge" d="M16 8h32v8H16c-3 0-5-2-5-4s2-4 5-4Zm0 56h32v8H16c-3 0-5-2-5-4s2-4 5-4Z" />
        <path class="jade-slip-groove" d="M19 30h26M19 39h26M19 48h26" />
        <path class="jade-slip-seal" d="m32 18 3 3-3 3-3-3 3-3Z" />
      </svg>`;
    ...
    this.session.disposers.push(() => this.clearPromptInjection());
  ...
  private async loadAppData(): Promise<void> {
    ...
    try {
      ...
      if (!new URLSearchParams(window.location.search).has('preview')) {
        ...
        this.newsPapers = parseNewsData(this.repository.getData('daoyuan_news_data')).papers;
      }
      this.sendContext();
  ...
  private async loadWorldData(): Promise<void> {
    ...
    for (const contact of rememberedContacts) {
      contactMap.set(contact.name, { ...contact, history: histories[contact.name] ?? [] });
    }
  ...
  private async generateTrendPosts(sourceMessageId = this.session.messageId ?? undefined): Promise<void> {
    ...
    try {
      ...
      this.trendPosts = retainTrendPosts([...this.trendPosts, ...posts], settings.maxPosts);
  ...
  private async generateForumContent(sourceMessageId = this.session.messageId ?? undefined): Promise<void> {
    if (this.forumGenerationInFlight) return; this.forumGenerationInFlight=true;
    try { const settings=readXianwangApiSettings(this.hostWindow ?? window); const input=await this.xianwangGenerationInput(sourceMessageId === undefined ? undefined : String(sourceMessageId)); input.existingTitles=this.forumPosts.map(p=>p.title); const posts=await generateForumPosts(settings,input,settings.forumBatchSize); this.forumPosts=retainNewest([...this.forumPosts,...posts],settings.forumMaxPosts); const existing=parseForumData(this.repository.getData('daoyuan_forum_data')); await this.repository.write('daoyuan_forum_data',{...existing,posts:this.forumPosts}); this.appData=this.repository.project(); this.frame?.contentWindow?.postMessage(makeBridgeMessage('event','FORUM_GENERATION_STATUS',{ok:true,posts:posts.length}),'*'); this.sendContext(); }
  ...
  private async generateNewsContent(sourceMessageId = this.session.messageId ?? undefined): Promise<void> {
    if(this.newsGenerationInFlight)return;this.newsGenerationInFlight=true;
    try{const settings=readXianwangApiSettings(this.hostWindow??window);const input=await this.xianwangGenerationInput(sourceMessageId===undefined?undefined:String(sourceMessageId));input.existingTitles=this.newsPapers.flatMap(p=>p.articles.map(a=>a.title));const papers=await generateNewsPapers(settings,input,settings.newsBatchSize);this.newsPapers=retainNewest([...this.newsPapers,...papers],settings.newsMaxPapers);const existing=parseNewsData(this.repository.getData('daoyuan_news_data'));await this.repository.write('daoyuan_news_data',{...existing,papers:this.newsPapers});this.appData=this.repository.project();this.frame?.contentWindow?.postMessage(makeBridgeMessage('event','NEWS_GENERATION_STATUS',{ok:true,papers:papers.length}),'*');this.sendContext();}
  ...
  private async handleMvuUpdateStarted(): Promise<void> {
    ...
    if (trendSettings.autoInterval > 0 && !trendsData.processedMessageIds.includes(messageId)) {
      const counter = trendsData.autoCounter + 1;
      const processedMessageIds = [...trendsData.processedMessageIds, messageId].slice(-200);
      await this.repository.write('daoyuan_web_trends_data', { ...trendsData, autoCounter: counter, processedMessageIds });
      if (counter >= trendSettings.autoInterval) {
        await this.repository.write('daoyuan_web_trends_data', { ...trendsData, autoCounter: 0, processedMessageIds });
        void this.generateTrendPosts(messageId);
      }
    }

    const forumData=parseForumData(this.repository.getData('daoyuan_forum_data'));
    if(trendSettings.forumAutoInterval>0&&!forumData.processedMessageIds.includes(messageId)){const counter=forumData.autoCounter+1;const processedMessageIds=[...forumData.processedMessageIds,messageId].slice(-200);await this.repository.write('daoyuan_forum_data',{...forumData,autoCounter:counter,processedMessageIds});if(counter>=trendSettings.forumAutoInterval){await this.repository.write('daoyuan_forum_data',{...forumData,autoCounter:0,processedMessageIds});void this.generateForumContent(messageId);}}
    const newsData=parseNewsData(this.repository.getData('daoyuan_news_data'));
    if(trendSettings.newsAutoInterval>0&&!newsData.processedMessageIds.includes(messageId)){const counter=newsData.autoCounter+1;const processedMessageIds=[...newsData.processedMessageIds,messageId].slice(-200);await this.repository.write('daoyuan_news_data',{...newsData,autoCounter:counter,processedMessageIds});if(counter>=trendSettings.newsAutoInterval){await this.repository.write('daoyuan_news_data',{...newsData,autoCounter:0,processedMessageIds});void this.generateNewsContent(messageId);}}
    ...
    if (beautySettings.autoInterval > 0 && !beautyProcessed.includes(messageId)) {
      ...
      await this.repository.write('daoyuan_web_beauty_data', { ...beautyData, autoCounter: counter, processedMessageIds });
      if (counter >= beautySettings.autoInterval) {
        await this.repository.write('daoyuan_web_beauty_data', { ...beautyData, autoCounter: 0, processedMessageIds });
  ...
  private resize(): void {
    ...
    const viewportWidth = this.hostWindow.visualViewport?.width ?? this.hostWindow.innerWidth;
    const viewportHeight = this.hostWindow.visualViewport?.height ?? this.hostWindow.innerHeight;
    if (this.orb && this.orbPosition) {
    ...
    const width = Math.min(420, viewportWidth, viewportHeight * IPHONE_RATIO);
    const height = width / IPHONE_RATIO;
    const left = this.shellPosition?.left ?? Math.max(0, viewportWidth - width - 24);
    const top = this.shellPosition?.top ?? Math.min(24, Math.max(0, viewportHeight - height));
    this.shellPosition = this.clampPosition(left, top, width, height, viewportWidth, viewportHeight);
    this.host.dataset.mode = 'phone';
  ...
  private positionOrb(): void {
    ...
    Object.assign(this.orb.style, { right: 'calc(14px + env(safe-area-inset-right))', bottom: 'calc(14px + env(safe-area-inset-bottom))', left: 'auto', top: 'auto' });
  }
