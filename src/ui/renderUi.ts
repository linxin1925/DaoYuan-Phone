...
interface YujianSettingsDraft { customPrompt: string; apiBaseUrl: string; apiKey: string; apiModel: string; }
interface BeautyApiSettingsDraft { apiBaseUrl: string; apiKey: string; apiModel: string; autoInterval: number; }
type ApiSettingsDraft = Pick<BeautyApiSettingsDraft, 'apiBaseUrl' | 'apiKey' | 'apiModel'>;
interface XianwangSettingsDraft extends ApiSettingsDraft { autoInterval: number; batchMin: number; batchMax: number; maxPosts: number; forumAutoInterval:number; forumBatchSize:number; forumMaxPosts:number; newsAutoInterval:number; newsBatchSize:number; newsMaxPapers:number; }
...
function appendContact(doc: Document, list: HTMLElement, contact: ContactPreview): void {
  ...
  meta.append(element(doc, 'span', 'contact-time', contact.time));
  if (contact.unread) meta.append(element(doc, 'span', 'contact-unread', String(contact.unread)));
  row.append(avatar, copy, meta);
  list.append(row);
}

function appendChatMessage(doc: Document, list: HTMLElement, message: ChatMessagePreview): void {
  const row = element(doc, 'div', `chat-message${message.from === 'me' ? ' is-me' : ''}`);
  row.append(element(doc, 'div', 'chat-bubble', message.text), element(doc, 'span', 'chat-message-time', message.time));
  list.append(row);
}
...
export function mountUi(doc: Document, sendToHost: (action: BridgeAction, payload?: Record<string, unknown>) => void): UiMount {
  ...
  let yujianSettings: YujianSettingsDraft = { customPrompt: '', apiBaseUrl: '', apiKey: '', apiModel: '' };
  let beautyApiSettings: BeautyApiSettingsDraft = { apiBaseUrl: '', apiKey: '', apiModel: '', autoInterval: 1 };
  let xianwangApiSettings: XianwangSettingsDraft = { apiBaseUrl: '', apiKey: '', apiModel: '', autoInterval: 3, batchMin: 2, batchMax: 3, maxPosts: 30, forumAutoInterval:3, forumBatchSize:2, forumMaxPosts:30, newsAutoInterval:5, newsBatchSize:1, newsMaxPapers:12 };
  ...
  function renderYujianSettings(content: HTMLElement): void {
    ...
    const field = (label: string, key: keyof YujianSettingsDraft, type = 'text'): HTMLInputElement => {
  ...
  function renderSettings(content: HTMLElement): void {
    ...
    content.append(grid, element(doc, 'p', 'notice muted', 'API 密钥仅保存在当前浏览器本地设置中，不写入聊天变量或模型提示词。'));
  }
  ...
  function renderXianwangApiSettings(content: HTMLElement): void {
    ...
    field('API 密钥（API Key）', 'apiKey', 'password');
    field('模型（Model）', 'apiModel');
    const numericField = (label: string, key: keyof Omit<XianwangSettingsDraft,keyof ApiSettingsDraft>, min: number, max: number): void => {
      const wrap = element(doc, 'label', 'settings-field');
      wrap.append(element(doc, 'span', 'settings-label', label));
  ...
  const renderPage = (content: HTMLElement): void => {
    ...
    if (active === 'yujian') {
      ...
      if (selectedContact) {
        ...
        chatHeader.append(person);
        chatPage.append(chatHeader);

        const messages = element(doc, 'div', 'chat-messages');
        const chatMessages = selectedContact.history?.length
          ? selectedContact.history
          : (previewChats[selectedContact.name] ?? [{ from: 'them', text: selectedContact.preview, time: selectedContact.time }]);
        chatMessages.forEach(message => appendChatMessage(doc, messages, message));
  ...
  const onClick = (event: Event): void => {
    ...
    if (action === 'app') {
      ...
      root.querySelectorAll<HTMLElement>('[data-setting]').forEach(node => {
        const key = node.dataset.setting as keyof YujianSettingsDraft;
      ...
      root.querySelectorAll<HTMLInputElement>('[data-xianwang-number-setting]').forEach(node => {
        const key = node.dataset.xianwangNumberSetting as keyof Omit<XianwangSettingsDraft,keyof ApiSettingsDraft>;
