...
export const ForumPostSchema = z.object({
  ...
  comments: z.array(ForumCommentSchema).max(20).default([]), generatedBy: z.enum(['ai', 'manual']).default('ai'), createdAt: z.string().datetime(), sourceMessageId: z.string().max(80).optional(),
});
export const ForumDataSchema = z.object({ posts: z.array(ForumPostSchema).max(500).default([]), autoCounter: z.number().int().nonnegative().default(0), processedMessageIds: z.array(z.string().max(80)).max(200).default([]) });
...
export const NewsPaperSchema = z.object({
  ...
  articles: z.array(NewsArticleSchema).min(2).max(8), letters: z.array(ReaderLetterSchema).max(12).default([]), generatedBy: z.literal('ai').default('ai'), createdAt: z.string().datetime(), sourceMessageId: z.string().max(80).optional(),
});
export const NewsDataSchema = z.object({ papers: z.array(NewsPaperSchema).max(200).default([]), autoCounter: z.number().int().nonnegative().default(0), processedMessageIds: z.array(z.string().max(80)).max(200).default([]) });
...
export function parseTrendsData(value: unknown): TrendsData {
  ...
  return result.success ? result.data : { posts: [], autoCounter: 0, processedMessageIds: [] };
}
...
export function parseNewsData(value: unknown): NewsData {
  ...
  return result.success ? result.data : { papers: [], autoCounter: 0, processedMessageIds: [] };
}
